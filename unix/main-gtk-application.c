...
/* Local patch: this multi-session GTK-application frontend has no
 * equivalent of main-gtk-simple.c's captured original argv, and a
 * restart here would need to account for every open session window,
 * not just one -- out of scope for a target this fork doesn't
 * actually build or ship (see unix/CMakeLists.txt: `putty' links
 * main-gtk-simple.c, not this file). Present only so config-gtk.c's
 * Design/Language panel still links if puttyapp/ptermapp are ever
 * built; the Apply & Restart button just won't do anything here. */
