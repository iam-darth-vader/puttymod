...
static int ngtkargs;
/* Local patch: unlike ngtkargs (just the count gtk_init swallowed),
 * this is the full length of gtkargvstart, i.e. every original
 * argv[1..] entry -- needed by restart_self_with_new_language() below
 * to relaunch with the exact command line this process was started
 * with. */
static int norigargs;
...
/* Local patch: Design/Language's "Apply & Restart" button
 * (unix/config-gtk.c, unix/platform.h). Unlike fork_and_exec_self()
 * above, this replaces the current process outright -- there's no
 * parent left waiting around, so there's nothing else to do to make
 * the old language go away. save_global_appearance() has already
 * written the newly selected language to ~/.putty/global-appearance
 * by the time this runs, which is what makes the freshly exec'd
 * process pick it up. */
static void do_restart_with_new_language(Conf *conf)
{
    char **args;
    int i;
    ...
    args = snewn(norigargs + 2, char *);
    args[0] = progname;
    for (i = 0; i < norigargs; i++)
        args[i+1] = gtkargvstart[i];
    args[norigargs+1] = NULL;
...
static void restart_lang_confirm_callback(void *vctx, int result)
{
    Conf *conf = (Conf *)vctx;
    if (result)
        do_restart_with_new_language(conf);
}

void restart_self_with_new_language(void *window, bool midsession, Conf *conf)
{
    if (midsession) {
        char *msg = dupprintf(
            "Restarting %s now will close the current session. Continue?",
            appname);
        create_message_box(
            (GtkWidget *)window, "Restart to change language", msg,
            string_width("Most of the width of the above text"),
            false, &buttons_yn, restart_lang_confirm_callback, conf);
        sfree(msg);
    } else {
        do_restart_with_new_language(conf);
    }
...
int main(int argc, char **argv)
{
    ...
    apply_global_window_theme();       /* local patch */

    /* Local patch: true only the very first time this Unix user has
     * ever run the app (no ~/.putty/global-appearance yet). Checked
     * here, before anything below has a chance to create that file. */
    first_run = !global_appearance_exists();
    ...
    if (argc > 1 && !strncmp(argv[1], "---", 3)) {
        ...
        if (first_run) {
            /* Local patch: seed a fresh install with Tokyo Night +
             * English rather than the plain compiled-in defaults, and
             * greet the user once. apply_theme_colours() persists this
             * to ~/.putty/global-appearance itself, which is also what
             * makes first_run false on every subsequent launch. */
            conf_set_int(conf, CONF_ui_language, 0);
            apply_theme_colours(conf, "7AA2F7", "1A1B26", "C0CAF5");
            welcome_box();
        }
