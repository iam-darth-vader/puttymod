...
GtkWidget *our_dialog_new(void)
{
    ...
    /*
     * Local patch: every dialog-family window (config box, About,
     * askpass, event log, host-CA config) goes through here, so this
     * is the one place to (a) tag them with a CSS class the two
     * dialog-chrome stylesheets (style.c, unix/global_appearance.c)
     * scope themselves to -- deliberately *not* a bare "window"
     * element selector, which would also match the terminal's own
     * toplevel and fight its independent RGBA transparency -- and (b)
     * give them an RGBA visual so a partially-transparent
     * background-color in that CSS (see apply_global_window_theme())
     * actually shows the desktop through, the same as the terminal.
     */
