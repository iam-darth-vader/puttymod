 ...
 * (unix/config-gtk.c). `window' is a GtkWidget* (declared void* here,
 * like about_box, so this can be called from config-gtk.c without it
 * needing to pull in GTK headers). If midsession, this must ask for
 * confirmation first, since restarting closes the current connection;
 * otherwise it can restart straight away since nothing is at risk. */
void restart_self_with_new_language(void *window, bool midsession,
                                    Conf *conf);
...
void welcome_box(void);        /* local patch: first-run only */
 ...
 */
/* Local patch: default to an installed, more legible mono font instead of
 * the generic "Monospace" alias (which resolves to DejaVu Sans Mono here). */
#define DEFAULT_GTK_CLIENT_FONT "client:JetBrainsMono Nerd Font Mono 11"
