...
/* Local patch: exposes the on-disk path of a saved session's settings
 * file, for session export/import (unix/session_export.c), which
 * reads/writes the raw file bytes directly rather than going through
 * a Conf -- that keeps export/import a lossless byte-for-byte copy of
 * whatever save_settings() already wrote, with no need to duplicate
 * or depend on the CONF_OPTION table's serialisation. */
char *session_file_path(const char *sessionname)
{
    return make_filename(INDEX_SESSION, sessionname);
}
