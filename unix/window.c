...
static Mouse_Button translate_button(Mouse_Button button)
{
    if (button == MBT_LEFT)
        return MBT_SELECT;
    /* Windows-style mouse buttons: right pastes, middle extends. */
...
static gint draw_area(GtkWidget *widget, cairo_t *cr, gpointer data)
{
    ...
    if (inst->surface) {
        ...
        /*
         * Blit with SOURCE rather than OVER, so the per-pixel alpha of
         * our backing surface reaches the (RGBA-visual) window intact
         * instead of being composited against an opaque backdrop.
         */
...
/*
 * Local patch: translucent terminal background. Only the *background*
 * fills are drawn with alpha; text and cursor stay fully opaque, so
 * glyphs remain crisp. Override at build time with -DPUTTY_BG_ALPHA=...
 */
#ifndef PUTTY_BG_ALPHA
#define PUTTY_BG_ALPHA 0.82
#endif
...
static void cairo_set_source_rgb_dim_alpha(cairo_t *cr, double r, double g,
                                           double b, bool dim, double alpha)
{
    if (dim)
        cairo_set_source_rgba(cr, r * 2 / 3, g * 2 / 3, b * 2 / 3, alpha);
    else
...
#ifdef DRAW_TEXT_CAIRO
    if (inst->uctx.type == DRAWTYPE_CAIRO) {
        /* Foreground drawing composites normally over the background. */
        cairo_set_operator(inst->uctx.u.cairo.cr, CAIRO_OPERATOR_OVER);
...
/*
 * Background-fill variants of the above. These use CAIRO_OPERATOR_SOURCE
 * so that the alpha *replaces* whatever was in the backing surface,
 * rather than compositing on top of it (which would accumulate towards
 * opaque on every repaint).
 */
static void draw_set_colour_bg(GtkFrontend *inst, int col, bool dim)
{
#ifdef DRAW_TEXT_CAIRO
    if (inst->uctx.type == DRAWTYPE_CAIRO) {
        cairo_set_operator(inst->uctx.u.cairo.cr, CAIRO_OPERATOR_SOURCE);
        cairo_set_source_rgb_dim_alpha(inst->uctx.u.cairo.cr,
                                       inst->cols[col].red / 65535.0,
                                       inst->cols[col].green / 65535.0,
                                       inst->cols[col].blue / 65535.0,
                                       dim, conf_get_int(inst->conf,
                                                         CONF_bg_alpha) / 100.0);
        return;
    }
#endif
    draw_set_colour(inst, col, dim);
}

static void draw_set_colour_rgb_bg(GtkFrontend *inst, optionalrgb orgb,
                                   bool dim)
{
#ifdef DRAW_TEXT_CAIRO
    if (inst->uctx.type == DRAWTYPE_CAIRO) {
        cairo_set_operator(inst->uctx.u.cairo.cr, CAIRO_OPERATOR_SOURCE);
        cairo_set_source_rgb_dim_alpha(inst->uctx.u.cairo.cr, orgb.r / 255.0,
                                       orgb.g / 255.0, orgb.b / 255.0,
                                       dim, conf_get_int(inst->conf,
                                                         CONF_bg_alpha) / 100.0);
        return;
    }
#endif
    draw_set_colour_rgb(inst, orgb, dim);
}
...
static void draw_backing_rect(GtkFrontend *inst)
{
    ...
    draw_set_colour_bg(inst, 258, false);
    ...
    unsigned long attr, int lattr, truecolour truecolour)
{
    ...
    if (truecolour.bg.enabled)
        draw_set_colour_rgb_bg(inst, truecolour.bg, attr & ATTR_DIM);
    else
        draw_set_colour_bg(inst, nbg, attr & ATTR_DIM);
...
static void start_backend(GtkFrontend *inst)
{
    ...
    /* Local patch: auto-fill the login password from the session, but only
     * when no private key is configured (a key must take precedence).
     * CONF_password holds an AES-encrypted blob (unix/pwcrypt.c); it is
     * decrypted here, in memory, only for this one use. */
    if (filename_is_null(conf_get_filename(inst->conf, CONF_keyfile))) {
        const char *stored = conf_get_str(inst->conf, CONF_password);
        if (stored && *stored) {
            char *autopw = decrypt_password_from_storage(stored);
            if (*autopw)
                cmdline_set_password(autopw);
            smemclr(autopw, strlen(autopw));
            sfree(autopw);
        }
    }
...
void new_session_window(Conf *conf, const char *geometry_string)
{
...
#if GTK_CHECK_VERSION(3,0,0)
    /* Local patch: don't let GTK paint an opaque theme background under us. */
    gtk_widget_set_app_paintable(GTK_WIDGET(inst->area), true);
#endif
    ...
    /*
     * Local patch: give the window an RGBA visual where the display
     * supports one, so the translucent background fills can actually
     * show the desktop through. Without a compositor this is a no-op
     * and we simply fall back to the normal opaque visual.
     */
#if GTK_CHECK_VERSION(3,0,0)
    {
        /*
         * Note: deliberately *not* gated on gdk_screen_is_composited().
         * Under XWayland (KDE Plasma on Wayland) the compositor manages
         * alpha at the Wayland level and never claims the X11
         * _NET_WM_CM_S0 selection, so GDK reports "not composited" even
         * though RGBA visuals work perfectly. Requiring that check would
         * silently disable transparency on exactly this setup.
         */
        GdkScreen *scr = gtk_widget_get_screen(GTK_WIDGET(inst->window));
        GdkVisual *rgba = scr ? gdk_screen_get_rgba_visual(scr) : NULL;
        if (rgba) {
            gtk_widget_set_visual(GTK_WIDGET(inst->window), rgba);
            gtk_widget_set_app_paintable(GTK_WIDGET(inst->window), true);
        }
    }
#endif
    {
