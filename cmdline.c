...
/* Local patch: let the GUI feed a session-stored password into the same
 * mechanism plink -pw uses. */
void cmdline_set_password(const char *pw)
{
    if (cmdline_password) {
        smemclr(cmdline_password, strlen(cmdline_password));
        sfree(cmdline_password);
