...
#include "storage.h"
#include "tree234.h"
/* Local patch: this file is nominally platform-independent, but this
 * fork only ever builds for Unix/GTK, and a handful of panel titles
 * need to translate a *template* ("Basic options for your %s
 * session") before substituting `appname' into it -- otherwise the
 * po file would have to be keyed on the literal post-substitution
 * text, which breaks (silently reverts to English) the instant
 * `appname' changes, exactly as happened when this fork was renamed
 * PuTTyMOD. Only I_() is used from here, never anything GTK-specific. */
#include "i18n.h"
...
struct sessionsaver_data {
    ...
    dlgcontrol *searchbox;              /* local patch: filter box */
    dlgcontrol *exportbutton, *importbutton;   /* local patch */
    dlgcontrol *netwarning;    /* local patch: "No internet connection" */
    struct sesslist sesslist;
    bool midsession;
    char *savedsession;     /* the current contents of ssd->editbox */
    /* Local patch: session-list search/filter. The listbox only shows
     * entries matching searchtext, so a listbox row index is no
     * longer the same as an index into sesslist.sessions -- visible[]
     * maps the former to the latter (built fresh every listbox
     * refresh); ALWAYS go through sessionsaver_resolve() rather than
     * indexing sesslist.sessions with a raw dlg_listbox_index()
     * result, or Load/Save/Delete will act on the wrong session. */
    char *searchtext;
    int *visible;
    int nvisible;
};

/* Local patch: ASCII-case-insensitive substring match for the session
 * search box. Deliberately byte-wise rather than using a Unicode
 * case-fold (e.g. GLib's g_utf8_casefold): this file is nominally
 * portable/GTK-independent (see the "config.c is portable" note
 * elsewhere in this codebase), so no GLib here. Non-ASCII bytes
 * (e.g. UTF-8 Cyrillic) pass through unchanged and still match
 * correctly on exact byte sequences -- just not case-folded, so
 * "россия" won't currently match "Россия". */
static bool ci_substr_match(const char *haystack, const char *needle)
{
    size_t nlen = strlen(needle), hlen = strlen(haystack), i;
    if (nlen == 0)
        return true;
    if (nlen > hlen)
        return false;
    for (i = 0; i + nlen <= hlen; i++) {
        ...
        bool match = true;
        for (j = 0; j < nlen; j++) {
            unsigned char a = haystack[i+j], b = needle[j];
            if (a >= 'A' && a <= 'Z') a += 32;
            if (b >= 'A' && b <= 'Z') b += 32;
...
static int sessionsaver_resolve(struct sessionsaver_data *ssd, int row)
{
    if (row < 0 || row >= ssd->nvisible)
        return -1;
    return ssd->visible[row];
}

/* Local patch: defined further down (with password_handler); forward
 * declared because sessionsaver_handler, above that point in the file,
 * needs to call it at every save/connect exit point. */
static void commit_password_encryption(Conf *conf);
    ...
    dlgparam *dlg, Conf *conf, bool *maybe_launch)
{
    ...
    int i = sessionsaver_resolve(ssd, row);
    bool isdef;
    if (i < 0) {
        dlg_beep(dlg);
        return false;
     ...
     * changing the value of the edit box. row (the listbox position),
     * not i (the resolved sesslist index), since the filter is still
     * in effect after the refresh above. */
                                 ...
                                 void *data, int event)
{
    ...
    if (event == EVENT_REFRESH) {
        ...
        } else if (ctrl == ssd->searchbox) {
            dlg_editbox_set(ctrl, dlg, ssd->searchtext ? ssd->searchtext : "");
        } else if (ctrl == ssd->listbox) {
            int i, row;
            const char *filter = ssd->searchtext ? ssd->searchtext : "";
            dlg_update_start(ctrl, dlg);
            dlg_listbox_clear(ctrl, dlg);
            sfree(ssd->visible);
            ssd->visible = snewn(ssd->sesslist.nsessions, int);
            ssd->nvisible = 0;
            row = 0;
            for (i = 0; i < ssd->sesslist.nsessions; i++) {
                if (*filter && !ci_substr_match(ssd->sesslist.sessions[i],
                                                filter))
                    continue;
                {
                    /* Local patch: "Default Settings" is a built-in
                     * sentinel name (settings.c/unix/storage.c), not
                     * an actual user session -- translate it for
                     * display only. Every OTHER comparison against
                     * this session still uses the real, untranslated
                     * ssd->sesslist.sessions[i] value, since that's
                     * the literal name the storage backend uses. */
                    const char *dispname = ssd->sesslist.sessions[i];
                    if (!strcmp(dispname, "Default Settings"))
                        dispname = I_("Default Settings");
                    dlg_listbox_add(ctrl, dlg, dispname);
                }
                {
                    /* Local patch: "Default Settings" has no detected
                     * host, so CONF_distro_icon is always empty for it
                     * and the row used to show no icon at all. Give it
                     * a generic question-mark icon instead, so the row
                     * isn't the only blank one in the list. Every
                     * OTHER row also gets an online/offline dot via
                     * dlg_listbox_set_icon_and_check() -- see its
                     * comment in dialog.h -- which is a superset of
                     * plain dlg_listbox_set_icon() and safe to call
                     * even when `ic' is empty (no distro icon known
                     * yet, e.g. never successfully connected): it
                     * still shows the dot on a blank base, it just
                     * skips the distro-icon part. */
                    bool isdef = !strcmp(ssd->sesslist.sessions[i],
                                         "Default Settings");
                    Conf *itmp = conf_new();
                    const char *ic, *host;
                    int port;
                    load_settings(ssd->sesslist.sessions[i], itmp);
                    ic = conf_get_str(itmp, CONF_distro_icon);
                    host = conf_get_str(itmp, CONF_host);
                    port = conf_get_int(itmp, CONF_port);
                    if (isdef) {
                        dlg_listbox_set_icon(ctrl, dlg, row, "question");
                    } else {
                        dlg_listbox_set_icon_and_check(
                            ctrl, dlg, row, (ic && *ic) ? ic : NULL,
                            ssd->sesslist.sessions[i], host, port);
                    }
                    conf_free(itmp);
                }
                ssd->visible[ssd->nvisible++] = i;
                row++;
            }
            dlg_update_done(ctrl, dlg);
        } else if (ctrl == ssd->okbutton) {
            /* Local patch: piggybacking the "No internet connection"
             * warning's visibility sync onto the Open/Apply button's
             * own EVENT_REFRESH -- not because it has anything to do
             * with that button, but because ctrl_text() controls
             * (ssd->netwarning) are created with handler==NULL and so
             * are silently skipped by dlg_refresh()'s full-dialog
             * broadcast (see the dialog.c gotcha this project has hit
             * more than once). Any sibling control that DOES have a
             * handler works as the hook; this one was picked because
             * its EVENT_REFRESH already fires at the right times for
             * free -- once at initial dialog construction (before the
             * window is shown, so this can never cause a visible
             * flash) and again on every dlg_refresh(NULL, dlg), e.g.
             * every category-tree click. The live/mid-session updates
             * (as a check actually completes) come from
             * status_check_finish() in unix/dialog.c calling
             * dlg_set_netwarning_visible() directly, not from here. */
            dlg_listbox_set_netwarning_ctrl(ssd->listbox, dlg,
                                            ssd->netwarning);
            dlg_set_netwarning_visible(
                ssd->netwarning, dlg,
                dlg_listbox_is_netdown(ssd->listbox, dlg));
        }
    } else if (event == EVENT_VALCHANGE) {
        int top, bottom, halfway, i;
        if (ctrl == ssd->searchbox) {
            sfree(ssd->searchtext);
            ssd->searchtext = dlg_editbox_get(ctrl, dlg);
            dlg_refresh(ssd->listbox, dlg);
        } else if (ctrl == ssd->editbox) {
            sfree(ssd->savedsession);
            ssd->savedsession = dlg_editbox_get(ctrl, dlg);
            /* This binary search assumes the listbox shows every
             * session at the same row as its sesslist index -- true
             * only with no search filter active (see ci_substr_match
             * above); skip it while filtered rather than risk
             * selecting an unrelated row. */
            if (ssd->searchtext && *ssd->searchtext)
                goto skip_jump_to_match;
        ...
        if (!ssd->midsession &&
             ...
             * contains a hostname.
             */
            bool loaded = load_selected_session(ssd, dlg, conf, &mbl);
            if (loaded &&
                (mbl && ctrl == ssd->listbox && conf_launchable(conf))) {
                detect_and_store_distro(ssd->savedsession, conf); /* local patch */
                commit_password_encryption(conf); /* local patch */
                save_global_appearance(conf); /* local patch */
                dlg_end(dlg, 1);       /* it's all over, and succeeded */
            } else if (loaded && ctrl == ssd->loadbutton) {
                /* Local patch: flash confirmation on the button
                 * itself -- only for an explicit Load click that
                 * didn't immediately launch (a double-click that DID
                 * launch has no time to see it anyway, and a
                 * double-click that merely populated the dialog has
                 * no "Load" button click to flash). */
                dlg_button_flash_success(ssd->loadbutton, dlg, "Loaded!");
            }
        } else if (ctrl == ssd->savebutton) {
            bool isdef = !strcmp(ssd->savedsession, "Default Settings");
            if (!ssd->savedsession[0]) {
                int i = sessionsaver_resolve(
                    ssd, dlg_listbox_index(ssd->listbox, dlg));
                if (i < 0) {
                    dlg_beep(dlg);
                    return;
                }
            ...
            save_global_appearance(conf); /* local patch */
            {
                ...
                if (errmsg) {
                    ...
                    /* Local patch: visible confirmation the save
                     * actually happened. */
                    dlg_button_flash_success(ssd->savebutton, dlg, "Saved!");
                }
            }
            get_sesslist(&ssd->sesslist, false);
            ...
            if (i <= 0) {
                ...
                get_sesslist(&ssd->sesslist, true);
                dlg_refresh(ssd->listbox, dlg);
                /* Local patch: visible confirmation the delete
                 * actually happened. */
                dlg_button_flash_success(ssd->delbutton, dlg, "Deleted!");
            }
        } else if (ctrl == ssd->exportbutton) {
            /* Local patch: export just the session currently selected
             * in the listbox, or -- if none is selected -- every
             * saved session except the "Default Settings" template
             * (which isn't really a session of its own). */
            int row = dlg_listbox_index(ssd->listbox, dlg);
            int sel = sessionsaver_resolve(ssd, row);
            if (sel >= 0 && strcmp(ssd->sesslist.sessions[sel],
                                   "Default Settings")) {
                const char *name = ssd->sesslist.sessions[sel];
                dlg_export_sessions_ui(dlg, &name, 1);
            } else {
                int k, n = 0;
                const char **names =
                    snewn(ssd->sesslist.nsessions, const char *);
                for (k = 0; k < ssd->sesslist.nsessions; k++)
                    if (strcmp(ssd->sesslist.sessions[k],
                              "Default Settings"))
                        names[n++] = ssd->sesslist.sessions[k];
                dlg_export_sessions_ui(dlg, names, n);
                sfree(names);
            }
        } else if (ctrl == ssd->importbutton) {
            /* Local patch: re-scan the sessions directory BEFORE
             * refreshing the listbox -- a newly imported session
             * wouldn't otherwise show up until some unrelated later
             * action forced another refresh (reported as "only shows
             * up after switching away and back"). */
            dlg_import_sessions_ui(dlg);
            get_sesslist(&ssd->sesslist, false);
            get_sesslist(&ssd->sesslist, true);
            dlg_refresh(ssd->listbox, dlg);
        } else if (ctrl == ssd->okbutton) {
...
/* Local patch: auto-login password. CONF_password always holds the
 * at-rest AES-encrypted blob (see unix/pwcrypt.c) or "" -- it is never
 * loaded back into the edit box, so a saved password can be replaced
 * but never displayed. What the user types goes into the transient,
 * never-saved CONF_password_new until commit_password_encryption()
 * (called from sessionsaver_handler, right before anything that saves
 * or connects) turns it into a fresh encrypted CONF_password. */
struct password_data {
    dlgcontrol *editbox, *statustext;
};

static void password_handler(dlgcontrol *ctrl, dlgparam *dlg,
                             void *data, int event)
{
    Conf *conf = (Conf *)data;
    struct password_data *pd =
        (struct password_data *)ctrl->context.p;

    if (event == EVENT_REFRESH) {
        if (ctrl == pd->editbox) {
            /* Local patch: CTRL_TEXT controls (like pd->statustext)
             * are always built with handler == NULL (see ctrl_text()
             * in dialog.c), so dlg_refresh()'s broadcast skips them
             * entirely -- "if (ctrl == pd->statustext)" here would be
             * dead code. Refresh it piggybacked on the editbox's own
             * refresh instead, since that control *does* have this
             * handler attached and reliably gets EVENT_REFRESH.
             *
             * The editbox itself must be re-set here to whatever's
             * CURRENTLY in CONF_password_new, NOT unconditionally to
             * "". dlg_refresh(NULL, dlg) -- a full-dialog refresh --
             * fires on plenty of completely routine events that have
             * nothing to do with this field: switching category in
             * the tree (treeselection_changed(), unix/dialog.c) does
             * it on every click, and GTK's gtk_entry_set_text() fires
             * its own "changed" signal even for a programmatic set,
             * which this dialog's EVENT_VALCHANGE handler (below)
             * faithfully writes into Conf. Blanking unconditionally
             * therefore silently discarded a password the instant the
             * user switched tabs (or the window was minimised and
             * restored) -- before they ever got a chance to click
             * Save. Setting it back to the string already held in
             * CONF_password_new is a no-op in effect (same value in,
             * same value out) but keeps whatever the user is
             * mid-typing alive across those refreshes; it still reads
             * as blank on a genuinely fresh/just-loaded session, since
             * CONF_password_new isn't a saved field and defaults to ""
             * there. The masked GtkEntry never shows plaintext either
             * way. */
            const char *typed = conf_get_str(conf, CONF_password_new);
            dlg_editbox_set(ctrl, dlg, typed ? typed : "");
            /* Local patch: a row of dots as a GTK *placeholder* (shown
             * only while the box is empty, never part of the actual
             * value -- see dlg_editbox_set_placeholder()) hinting that
             * a password is already saved, without revealing its real
             * length. Disappears the moment the user types, same as
             * the status text below just says in words. */
            dlg_editbox_set_placeholder(
                ctrl, dlg,
                has_stored_password(conf) ? "\xe2\x80\xa2\xe2\x80\xa2\xe2\x80\xa2\xe2\x80\xa2\xe2\x80\xa2\xe2\x80\xa2\xe2\x80\xa2\xe2\x80\xa2" : NULL);
            dlg_text_set(pd->statustext, dlg,
                        has_stored_password(conf) ?
                        "A password is saved (encrypted for this"
                        " user). Type a new one to replace it." :
                        "No password is set.");
        }
    } else if (event == EVENT_VALCHANGE) {
        if (ctrl == pd->editbox) {
            char *typed = dlg_editbox_get(ctrl, dlg);
            conf_set_str(conf, CONF_password_new, typed);
            sfree(typed);
        }
    }
}
...
static void commit_password_encryption(Conf *conf)
{
    const char *newpw = conf_get_str(conf, CONF_password_new);
    if (newpw && *newpw) {
        char *enc = encrypt_password_for_storage(newpw);
        conf_set_str(conf, CONF_password, enc);
        sfree(enc);
        conf_set_str(conf, CONF_password_new, "");
    }
}

/* Local patch: accent/background/text colour pickers in
 * Design/Window. A regular person can't be expected to type hex
 * codes (fair complaint), so each of the three colours gets a status
 * text showing the current value plus a button that opens the native
 * GTK colour-chooser dialog, exactly the same dlg_coloursel_start()/
 * dlg_coloursel_results() flow the ANSI colour grid above uses --
 * just against a CONF_TYPE_STR hex field instead of a CONF_colours
 * RGB triple. */
struct wintheme_data {
    dlgcontrol *statustext, *button;
    int confid;
};

static void wintheme_hex_to_rgb(const char *hex, int *r, int *g, int *b)
{
    int v[6], i;
    *r = *g = *b = 0;
    if (!hex || strlen(hex) != 6)
...
static void wintheme_handler(dlgcontrol *ctrl, dlgparam *dlg,
                             void *data, int event)
{
    Conf *conf = (Conf *)data;
    struct wintheme_data *wd =
        (struct wintheme_data *)ctrl->context.p;

    /*
     * statustext shows only "#RRGGBB" -- a bare hex code, not a
     * sentence -- deliberately kept out of the translated label next
     * to it (a separate, static ctrl_text created once and never
     * touched again). Concatenating the two into one dynamic string
     * would defeat I_()'s exact-match lookup on the label half, since
     * "<label>: #RRGGBB" never matches any single po entry once the
     * hex half varies.
     *
     * The refresh itself is piggybacked on wd->button rather than
     * keyed on wd->statustext: CTRL_TEXT controls are always built
     * with handler == NULL (see ctrl_text() in dialog.c), so
     * dlg_refresh()'s broadcast never delivers EVENT_REFRESH to them
     * at all -- "if (ctrl == wd->statustext)" here would be dead
     * code, silently leaving the hex code blank on every dialog open.
     */
    if (event == EVENT_REFRESH) {
        if (ctrl == wd->button) {
            char *txt = dupprintf("#%s", conf_get_str(conf, wd->confid));
            dlg_text_set(wd->statustext, dlg, txt);
            sfree(txt);
        }
    } else if (event == EVENT_ACTION) {
        if (ctrl == wd->button) {
            int r, g, b;
            wintheme_hex_to_rgb(conf_get_str(conf, wd->confid), &r, &g, &b);
            dlg_coloursel_start(ctrl, dlg, r, g, b);
        }
    } else if (event == EVENT_CALLBACK) {
        if (ctrl == wd->button) {
            int r, g, b;
            if (dlg_coloursel_results(ctrl, dlg, &r, &g, &b)) {
                char *hex = dupprintf("%02X%02X%02X", r, g, b);
                char *txt;
                conf_set_str(conf, wd->confid, hex);
                txt = dupprintf("#%s", hex);
                dlg_text_set(wd->statustext, dlg, txt);
                sfree(txt);
                sfree(hex);
...
/* Local patch: one-click theme presets (Design/Themes). Sets the
 * dialog's accent/bg/fg AND the terminal's core colours (default fg/
 * bg, bold fg/bg, cursor text/colour -- CONF_colours slots 0-5) in
 * one go; deliberately leaves the 16 ANSI palette entries (slots
 * 6-21) untouched, since those are more likely to be something the
 * user tuned deliberately and a preset shouldn't clobber silently. */
struct theme_preset {
    const char *name;
    const char *accent, *bg, *fg;
};
static const struct theme_preset theme_presets[] = {
    { "Dracula", "BD93F9", "282A36", "F8F8F2" },
    { "Nord", "88C0D0", "2E3440", "D8DEE9" },
    { "Solarized Dark", "268BD2", "002B36", "839496" },
    { "Solarized Light", "268BD2", "FDF6E3", "657B83" },
    { "Gruvbox Dark", "FE8019", "282828", "EBDBB2" },
    { "One Dark", "61AFEF", "282C34", "ABB2BF" },
    { "Monokai", "F92672", "272822", "F8F8F2" },
    { "Tokyo Night", "7AA2F7", "1A1B26", "C0CAF5" },
    { "Catppuccin Mocha", "89B4FA", "1E1E2E", "CDD6F4" },
    { "Catppuccin Latte", "1E66F5", "EFF1F5", "4C4F69" },
    { "Ayu Dark", "FFB454", "0F1419", "E6E1CF" },
    { "Material Palenight", "82AAFF", "292D3E", "A6ACCD" },
    { "Everforest Dark", "A7C080", "2D353B", "D3C6AA" },
    { "Rose Pine", "EBBCBA", "191724", "E0DEF4" },
    { "Night Owl", "82AAFF", "011627", "D6DEEB" },
    { "GitHub Dark", "58A6FF", "0D1117", "C9D1D9" },
};

struct theme_data {
    dlgcontrol *listbox, *button, *nameedit, *savebutton;
    /* Custom (user-saved) presets loaded fresh each time the listbox
     * is refreshed, so by the time Apply/Save is clicked this
     * reflects whatever the listbox is currently showing -- indices
     * >= lenof(theme_presets) in the listbox map to customs[index -
     * lenof(theme_presets)]. */
    struct custom_theme customs[MAX_CUSTOM_THEMES];
    int ncustoms;
    /* Local patch: the Design/Window colour-picker buttons (their
     * wintheme_data::button, NOT the read-only statustext -- ctrl_text
     * has a NULL handler and dlg_refresh() silently no-ops on it, same
     * dead-handler trap as elsewhere in this file), stashed here so
     * "Apply preset" can refresh just these three instead of calling
     * dlg_refresh(NULL, dlg). A full-dialog refresh reaches every
     * control including the auto-login password editbox, whose own
     * EVENT_REFRESH unconditionally blanks it via dlg_editbox_set --
     * and GTK's entry "changed" signal fires on that programmatic set
     * too, so it wasn't just a display glitch: it silently overwrote
     * CONF_password_new with "", wiping a password the user had typed
     * but not yet saved. */
    dlgcontrol *colorbuttons[3];
    int ncolorbuttons;
};

static void set_colour_slot(Conf *conf, int idx, int r, int g, int b)
{
...
void apply_theme_colours(Conf *conf, const char *accent,
                         const char *bg, const char *fg)
{
    int fr, fgv, fb, br, bgv, bb, ar, ag, ab;
    wintheme_hex_to_rgb(fg, &fr, &fgv, &fb);
    wintheme_hex_to_rgb(bg, &br, &bgv, &bb);
    wintheme_hex_to_rgb(accent, &ar, &ag, &ab);

    conf_set_str(conf, CONF_win_accent, accent);
    conf_set_str(conf, CONF_win_bg, bg);
    conf_set_str(conf, CONF_win_fg, fg);

    set_colour_slot(conf, 0, fr, fgv, fb);   /* Default Foreground */
    set_colour_slot(conf, 1, fr, fgv, fb);   /* Default Bold Foreground */
    set_colour_slot(conf, 2, br, bgv, bb);   /* Default Background */
    set_colour_slot(conf, 3, br, bgv, bb);   /* Default Bold Background */
    set_colour_slot(conf, 4, br, bgv, bb);   /* Cursor Text */
    set_colour_slot(conf, 5, ar, ag, ab);    /* Cursor Colour */

    save_global_appearance(conf);
    apply_global_window_theme();
}

static void theme_handler(dlgcontrol *ctrl, dlgparam *dlg,
                          void *data, int event)
{
    Conf *conf = (Conf *)data;
    struct theme_data *td = (struct theme_data *)ctrl->context.p;
    ...
    if (event == EVENT_REFRESH) {
        if (ctrl == td->listbox) {
            ...
            dlg_update_start(ctrl, dlg);
            dlg_listbox_clear(ctrl, dlg);
            for (i = 0; i < lenof(theme_presets); i++)
                dlg_listbox_add(ctrl, dlg, theme_presets[i].name);
            td->ncustoms = load_custom_themes(td->customs,
                                              MAX_CUSTOM_THEMES);
            for (j = 0; j < td->ncustoms; j++)
                dlg_listbox_add(ctrl, dlg, td->customs[j].name);
            dlg_update_done(ctrl, dlg);
        } else if (ctrl == td->nameedit) {
            dlg_editbox_set(ctrl, dlg, "");
        }
    } else if (event == EVENT_ACTION) {
        if (ctrl == td->button) {
            int i = dlg_listbox_index(td->listbox, dlg);
            if (i < 0) {
                dlg_beep(dlg);
                return;
            }
            if ((size_t)i < lenof(theme_presets)) {
                const struct theme_preset *p = &theme_presets[i];
                apply_theme_colours(conf, p->accent, p->bg, p->fg);
            } else {
                struct custom_theme *c =
                    &td->customs[i - lenof(theme_presets)];
                apply_theme_colours(conf, c->accent, c->bg, c->fg);
            }
            /* Local patch: refresh only the three colour-readout
             * buttons this changed, NOT dlg_refresh(NULL, dlg) -- see
             * the comment on theme_data::colorbuttons for why a full
             * refresh silently wiped an in-progress auto-login
             * password. */
            {
                int ci;
                for (ci = 0; ci < td->ncolorbuttons; ci++)
                    dlg_refresh(td->colorbuttons[ci], dlg);
            }
        } else if (ctrl == td->savebutton) {
            char *name = dlg_editbox_get(td->nameedit, dlg);
            if (!*name) {
                sfree(name);
                dlg_beep(dlg);
                return;
            }
            save_custom_theme(name, conf_get_str(conf, CONF_win_accent),
                              conf_get_str(conf, CONF_win_bg),
                              conf_get_str(conf, CONF_win_fg));
            sfree(name);
            dlg_refresh(td->listbox, dlg);
            dlg_refresh(td->nameedit, dlg);
        }
    }
}

/* Local patch: "Обновить превью" button in Оформление/Терминал. The
 * preview drawing area (context passed as P(c)) has no live-update
 * wiring to the other controls in the panel, so this is how the user
 * asks it to redraw with whatever font/colours/cursor/alpha are
 * currently set. */
static void appearance_preview_refresh_handler(dlgcontrol *ctrl,
                                                dlgparam *dlg,
                                                void *data, int event)
{
    if (event == EVENT_ACTION)
        dlg_text_preview_refresh((dlgcontrol *)ctrl->context.p, dlg);
}

/* Local patch: "Применить" button in both Оформление panels. Persists
 * the current font/colours/cursor/alpha/window-theme values to
 * ~/.putty/global-appearance immediately (same write that otherwise
 * only happens implicitly on Save/Connect), and re-applies the dialog
 * CSS live so a changed accent/bg/fg colour is visible at once without
 * restarting the app. */
static void appearance_apply_handler(dlgcontrol *ctrl, dlgparam *dlg,
                                     void *data, int event)
{
    Conf *conf = (Conf *)data;
    if (event == EVENT_ACTION) {
                      ...
                      int protocol, int protcfginfo)
{
    ...
    /* Local patch: "No internet connection" -- hidden by default,
     * shown as a red warning badge (see unix/style.c's
     * ".putty-netwarning" rule) only when GNetworkMonitor
     * (unix/dialog.c) reports THIS machine has no working network --
     * distinct from any individual saved session actually being down,
     * even though every session's dot also turns red at the same time
     * (deliberately: this text is what tells the user WHY, so an
     * all-red list doesn't read as "all N of my servers just died").
     * Wired up from sessionsaver_handler's EVENT_REFRESH below, not
     * here -- at this point in setup_config_box() the dialog is still
     * just an abstract control tree, with no GTK widget yet to
     * show/hide. The warning symbol is baked into the msgid itself
     * (translators keep it as-is) rather than prepended in code,
     * since ctrl_text()'s label is translated as one whole string at
     * render time (see unix/i18n.c). */
    ssd->netwarning = ctrl_text(s, "\xE2\x9A\xA0 No internet connection",
                                HELPCTX(no_help));
    ssd->netwarning->column = 1 | (1 << 16);   /* columns 1-2 */

    /*
     * The Session panel.
     */
    str = dupprintf(I_("Basic options for your %s session"), appname);
    ...
    ctrl_columns(s, 1, 100);
    /* Local patch: filter box, narrows the list below to matching
     * names as you type -- separate from ssd->editbox above, which is
     * for typing a name to save/load under and already has its own
     * jump-to-nearest-match behaviour (skipped while this filter is
     * active, see sessionsaver_handler). */
    ssd->searchbox = ctrl_editbox(s, "Search sessions:", NO_SHORTCUT, 100,
                                  HELPCTX(session_saved),
                                  sessionsaver_handler, P(ssd), P(NULL));
    ctrl_columns(s, 2, 75, 25);
    ...
    ssd->listbox->listbox.icons = true;  /* local patch */
    if (!midsession) {
    ...
    ctrl_columns(s, 1, 100);
    /* Local patch: export/import the saved-sessions list (or just the
     * one selected in the listbox above) as a single ".pex" file, for
     * moving profiles between machines -- allowed mid-session too,
     * since neither touches the profile currently being edited. */
    ctrl_columns(s, 2, 50, 50);
    ssd->exportbutton = ctrl_pushbutton(s, "Export...", NO_SHORTCUT,
                                        HELPCTX(session_saved),
                                        sessionsaver_handler, P(ssd));
    ssd->exportbutton->column = 0;
    ssd->importbutton = ctrl_pushbutton(s, "Import...", NO_SHORTCUT,
                                        HELPCTX(session_saved),
                                        sessionsaver_handler, P(ssd));
    ssd->importbutton->column = 1;
    ctrl_columns(s, 1, 100);
    ...
    /*
     * Local patch: theme_data for Design/Themes is allocated below,
     * but the Design/Window block further down (which builds the
     * three colour-picker buttons) needs to stash pointers into it
     * too -- hence hoisting the variable this far up. See
     * theme_data::colorbuttons for why.
     */
    struct theme_data *theme_td = NULL;

    /*
     * Local patch: the "Design" category (shown as "Оформление" once
     * translated). Profile-independent appearance settings -- font,
     * colours, cursor, transparency, and the dialog's own accent/bg/fg
     * colours. These are edited here via the normal Conf-bound controls
     * (same CONF_font/CONF_colours/etc. fields the Terminal/Window
     * panels use further down), but unlike everything else in this
     * dialog, they are NOT part of the session:
     * apply_global_appearance()/save_global_appearance() (settings.c,
     * config.c's sessionsaver_handler) overlay them onto every session
     * on load and persist them to ~/.putty/global-appearance on connect,
     * so they follow the user rather than the profile. Subpaths are
     * named "Design/Terminal" and "Design/Window" so their leaf
     * labels ("Terminal", "Window") reuse the SAME translation as the
     * unrelated top-level panels of the same name further down --
     * deliberate, not a collision, since I_() looks up by string value.
     */
    ctrl_settitle(b, "Design",
                  "Design settings, applied identically to every profile");

    ctrl_settitle(b, "Design/Themes",
                  "One-click colour presets for the dialog and terminal");
    s = ctrl_getset(b, "Design/Themes", "presets", NULL);
    ctrl_text(s, "Sets the accent/background/text colours above and the"
              " terminal's core colours (background, text, cursor) in one"
              " go. The 16 ANSI colours are left as they are.",
              HELPCTX(no_help));
    {
        theme_td = (struct theme_data *)
            ctrl_alloc(b, sizeof(struct theme_data));
        theme_td->ncolorbuttons = 0;
        ctrl_columns(s, 2, 70, 30);
        theme_td->listbox = ctrl_listbox(s, "Preset:", NO_SHORTCUT,
                                   HELPCTX(no_help), theme_handler, P(theme_td));
        theme_td->listbox->column = 0;
        theme_td->listbox->listbox.height = 8;
        theme_td->button = ctrl_pushbutton(s, "Apply preset", NO_SHORTCUT,
                                     HELPCTX(no_help), theme_handler, P(theme_td));
        theme_td->button->column = 1;
        ctrl_columns(s, 1, 100);

        ctrl_text(s, "Like your current colours? Save them here under"
                  " your own name to reuse or share.", HELPCTX(no_help));
        ctrl_columns(s, 2, 70, 30);
        theme_td->nameedit = ctrl_editbox(s, "New preset name:", NO_SHORTCUT, 60,
                                    HELPCTX(no_help), theme_handler,
                                    P(theme_td), P(NULL));
        theme_td->nameedit->column = 0;
        theme_td->savebutton = ctrl_pushbutton(s, "Save as...", NO_SHORTCUT,
                                         HELPCTX(no_help), theme_handler,
                                         P(theme_td));
        theme_td->savebutton->column = 1;
        ctrl_columns(s, 1, 100);
    }

    ctrl_settitle(b, "Design/Terminal",
                  "Font, colours, cursor and transparency of the"
                  " terminal window");
    s = ctrl_getset(b, "Design/Terminal", "preview", "Preview");
    c = ctrl_text(s, "", HELPCTX(no_help));
    c->text.preview = true;
    /* Both buttons sit at the TOP of the panel, beside the preview: the
     * colour list further down makes this panel taller than the dialog,
     * so anything appended at the bottom would be scrolled out of sight. */
    ctrl_columns(s, 2, 50, 50);
    {
        dlgcontrol *pb = ctrl_pushbutton(
            s, "Refresh preview", NO_SHORTCUT, HELPCTX(no_help),
            appearance_preview_refresh_handler, P(c));
        pb->column = 0;
    }
    {
        dlgcontrol *ab = ctrl_pushbutton(
            s, "Apply to all profiles", NO_SHORTCUT, HELPCTX(no_help),
            appearance_apply_handler, P(NULL));
        ab->column = 1;
    }
    ctrl_columns(s, 1, 100);

    s = ctrl_getset(b, "Design/Terminal", "font", "Font settings");
    ctrl_fontsel(s, "Font used in the terminal window", NO_SHORTCUT,
                HELPCTX(no_help), conf_fontsel_handler, I(CONF_font));

    s = ctrl_getset(b, "Design/Terminal", "cursor",
                    "Adjust the use of the cursor");
    ctrl_radiobuttons(s, "Cursor appearance:", NO_SHORTCUT, 3,
                      HELPCTX(no_help),
                      conf_radiobutton_handler,
                      I(CONF_cursor_type),
                      "Block", NO_SHORTCUT, I(CURSOR_BLOCK),
                      "Underline", NO_SHORTCUT, I(CURSOR_UNDERLINE),
                      "Vertical line", NO_SHORTCUT,
                      I(CURSOR_VERTICAL_LINE));
    ctrl_checkbox(s, "Cursor blinks", NO_SHORTCUT,
                  HELPCTX(no_help),
                  conf_checkbox_handler, I(CONF_blink_cur));

    s = ctrl_getset(b, "Design/Terminal", "alpha",
                    "Background transparency");
    ctrl_editbox(s, "Background opacity, % (100 = opaque)",
                NO_SHORTCUT, 20, HELPCTX(no_help),
                conf_editbox_handler, I(CONF_bg_alpha), ED_INT);

    s = ctrl_getset(b, "Design/Terminal", "colours",
                    "Options controlling use of colours");
    ctrl_columns(s, 2, 67, 33);
    cd = (struct colour_data *)ctrl_alloc(b, sizeof(struct colour_data));
    cd->listbox = ctrl_listbox(s, "Select a colour to adjust:", NO_SHORTCUT,
                               HELPCTX(no_help), colour_handler, P(cd));
    cd->listbox->column = 0;
    cd->listbox->listbox.height = 7;
    c = ctrl_text(s, "RGB value:", HELPCTX(no_help));
    c->column = 1;
    cd->redit = ctrl_editbox(s, "Red", NO_SHORTCUT, 50, HELPCTX(no_help),
                             colour_handler, P(cd), P(NULL));
    cd->redit->column = 1;
    cd->gedit = ctrl_editbox(s, "Green", NO_SHORTCUT, 50, HELPCTX(no_help),
                             colour_handler, P(cd), P(NULL));
    cd->gedit->column = 1;
    cd->bedit = ctrl_editbox(s, "Blue", NO_SHORTCUT, 50, HELPCTX(no_help),
                             colour_handler, P(cd), P(NULL));
    cd->bedit->column = 1;
    cd->button = ctrl_pushbutton(s, "Modify", NO_SHORTCUT,
                                 HELPCTX(no_help), colour_handler, P(cd));
    cd->button->column = 1;
    ctrl_columns(s, 1, 100);
    /* Local patch: this panel is tall enough to scroll, so the button
     * at the top (beside the preview) is out of view by the time the
     * user has scrolled down to the colour list -- repeat it here so
     * an Apply control is always visible near whatever was just
     * changed. */
    ctrl_pushbutton(s, "Apply to all profiles", NO_SHORTCUT,
                    HELPCTX(no_help), appearance_apply_handler, P(NULL));

    ctrl_settitle(b, "Design/Window",
                  "Colours of the settings window itself (independent of"
                  " the system theme)");
    s = ctrl_getset(b, "Design/Window", "theme", NULL);
    ctrl_text(s, "Pick colours visually below, or set the window's"
              " transparency. Click Apply to save and apply immediately.",
              HELPCTX(no_help));
    {
        static const struct { const char *label; int confid; } wt[] = {
            { "Accent colour", CONF_win_accent },
            { "Window background", CONF_win_bg },
            { "Text colour", CONF_win_fg },
        };
        size_t wi;
        for (wi = 0; wi < lenof(wt); wi++) {
            struct wintheme_data *wd = (struct wintheme_data *)
                ctrl_alloc(b, sizeof(struct wintheme_data));
            dlgcontrol *label;
            wd->confid = wt[wi].confid;
            ctrl_columns(s, 3, 45, 25, 30);
            label = ctrl_text(s, wt[wi].label, HELPCTX(no_help));
            label->column = 0;
            wd->statustext = ctrl_text(s, "", HELPCTX(no_help));
            wd->statustext->column = 1;
            wd->button = ctrl_pushbutton(s, "Choose...", NO_SHORTCUT,
                                         HELPCTX(no_help),
                                         wintheme_handler, P(wd));
            wd->button->column = 2;
            ctrl_columns(s, 1, 100);
            /* Local patch: let Design/Themes's "Apply preset" refresh
             * just this button (see theme_data::colorbuttons) instead
             * of the whole dialog. */
            if (theme_td &&
                theme_td->ncolorbuttons < (int)lenof(theme_td->colorbuttons))
                theme_td->colorbuttons[theme_td->ncolorbuttons++] = wd->button;
        }
    }
    ctrl_fontsel(s, "Interface font (labels, buttons, tree)", NO_SHORTCUT,
                HELPCTX(no_help), conf_fontsel_handler, I(CONF_win_font));
    ctrl_editbox(s, "Window opacity, % (100 = opaque)", NO_SHORTCUT, 20,
                HELPCTX(no_help),
                conf_editbox_handler, I(CONF_win_alpha), ED_INT);
    ctrl_pushbutton(s, "Apply to all profiles",
                    NO_SHORTCUT, HELPCTX(no_help),
                    appearance_apply_handler, P(NULL));

    /*
     * Local patch: interface language, moved out of Design/Window into
     * its own panel since it has nothing to do with window colours and
     * was getting lost among them. The actual controls are added from
     * unix/config-gtk.c (gtk_setup_config_box), since they're tied to
     * this fork's own GTK-only i18n system (unix/i18n.c) -- this call
     * just registers the panel so that code has somewhere to put them.
     */
    ctrl_settitle(b, "Design/Language",
                  "Choose the language used for this dialog's own"
                  " interface");
     ...
     * The Window panel.
     */
    str = dupprintf(I_("Options controlling %s's window"), appname);
     ...
     */
    str = dupprintf(I_("Configure the appearance of %s's window"), appname);
     ...
     */
    str = dupprintf(I_("Configure the behaviour of %s's window"), appname);
    ...
    str = dupprintf(I_("Adjust how %s handles line drawing characters"), appname);
    ...
    /*
     * Local patch: the actual RGB-value adjustment grid used to live
     * here too (a "Window/Colours" / adjust set, identical in spirit
     * to the one now in Design/Terminal). Since colour values are now
     * profile-independent and edited exclusively from Design/Terminal
     * (see setup_config_box's caller in config.c's Design block), that
     * duplicate grid has been removed from here; only the ANSI/xterm/
     * bold-style *usage* toggles above remain, since those have no
     * equivalent in Design and aren't colour values as such.
     */
    ...
    if (protocol >= 0) {
        ...
        if (!midsession) {
            ...
            {
                /* Local patch: auto-login password (masked, AES-encrypted
                 * at rest -- see unix/pwcrypt.c). Ignored if a private key
                 * is configured. Never shown again once saved: the box
                 * always starts empty, and typing in it only replaces the
                 * stored password (via commit_password_encryption()),
                 * it never reveals the old one. */
                struct password_data *pd = (struct password_data *)
                    ctrl_alloc(b, sizeof(struct password_data));
                pd->editbox = ctrl_editbox(
                    s, "Auto-login password (ignored if a key is set)",
                    NO_SHORTCUT, 50, HELPCTX(connection_username),
                    password_handler, P(pd), P(NULL));
                pd->editbox->editbox.password = true;
                pd->statustext = ctrl_text(s, "", HELPCTX(connection_username));
                pd->statustext->context.p = pd;
                pd->statustext->handler = password_handler;
            }
            {
