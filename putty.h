...
void apply_global_appearance(Conf *conf);      /* local patch */
void save_global_appearance(Conf *conf);       /* local patch */
void apply_global_window_theme(void);          /* local patch */
/* Local patch: true iff ~/.putty/global-appearance has never been
 * written -- i.e. this is the very first time the app has run for
 * this Unix user. Used to seed a default theme/language and show a
 * one-time welcome dialog. */
bool global_appearance_exists(void);
/* Local patch: apply a "Design/Themes" preset (accent/bg/fg as
 * "RRGGBB" hex strings) to conf's window-chrome colours AND the
 * terminal's core colour slots 0-5, then persist it. Exposed (was
 * static in config.c) so unix/main-gtk-simple.c can seed the
 * first-run default theme before any dialog is shown. */
void apply_theme_colours(Conf *conf, const char *accent,
                         const char *bg, const char *fg);
/* Local patch: user-saved theme presets (Design/Themes "Save as..."),
 * ~/.putty/custom-themes, one "Name=accent,bg,fg" line per preset. */
#define MAX_CUSTOM_THEMES 64
struct custom_theme {
    char name[64];
    char accent[8], bg[8], fg[8];
};
int load_custom_themes(struct custom_theme *out, int max);
void save_custom_theme(const char *name, const char *accent,
                       const char *bg, const char *fg);
char *encrypt_password_for_storage(const char *plaintext);  /* local patch */
char *decrypt_password_from_storage(const char *stored);    /* local patch */
bool has_stored_password(Conf *conf);                        /* local patch */
char *session_file_path(const char *sessionname);            /* local patch */
/* Local patch: session export/import (unix/session_export.c). Bundles
 * the raw on-disk settings files for `names' (nnames of them) into
 * one ".pex" file, optionally password-protected (AES-256-CBC with an
 * Argon2id-derived key -- see the .c file for why this is a genuinely
 * separate, stronger scheme than the fixed-per-user-key one
 * pwcrypt.c uses for the auto-login password: an export is meant to
 * leave the machine, so there's no local key file to lean on, and
 * the whole point is that it must be undecryptable without the
 * password even by someone who has fully compromised this binary).
 * Returns false and fills *errmsg (caller must sfree it) on failure. */
bool export_sessions(const char *const *names, int nnames,
                     const char *password, const char *destpath,
                     char **errmsg);

typedef enum { PEX_PLAIN, PEX_ENCRYPTED, PEX_BAD_FILE } PexFileKind;
/* Peeks at srcpath's header only, to tell the UI whether to prompt for
 * a password before calling import_sessions(). */
PexFileKind export_file_kind(const char *srcpath, char **errmsg);

/* Called (if non-NULL) when one or more sessions in the bundle would
 * overwrite an existing saved session of the same name, listing
 * `names' (ncollide of them, borrowed pointers, valid only for the
 * duration of the call). Return true to proceed and overwrite all of
 * them, false to cancel the whole import (all-or-nothing -- simpler
 * and less surprising than a partial import). */
typedef bool (*pex_confirm_overwrite_fn)(void *ctx, const char *const *names,
                                         int ncollide);

/* Imports every session bundled in srcpath (written by export_sessions
 * above). password may be NULL/"" only if the file wasn't encrypted;
 * for an encrypted file, a wrong password is reported cleanly via
 * *wrong_password (PKCS#7 padding check) rather than importing
 * garbage, so the caller can re-prompt. On success, *nimported is set
 * (0 imports with no error is a legitimately empty bundle, not a
 * failure). If the user declines the overwrite-confirmation callback
 * above, returns false with *errmsg left NULL (a cancel, not an
 * error) -- the caller shouldn't show an error dialog for that case,
 * since it already showed its own confirm dialog and got "no". */
...
void cmdline_set_password(const char *pw);   /* local patch */
