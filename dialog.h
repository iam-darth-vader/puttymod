...
struct dlgcontrol {
    ...
    union {
        ...
        struct { /* for CTRL_LISTBOX */
             ...
             */
            bool hscroll;
            /* Local patch: per-item distro/OS icon column. */
            bool icons;
        } listbox;
        ...
        struct { /* for CTRL_TEXT */
             ...
             */
            bool wrap;
            /* Local patch: render a live sample-text/cursor preview
             * (GtkDrawingArea) instead of a label. */
            bool preview;
        } text;
    };
};
...
/* Local patch: greyed-out hint shown only while the box is empty,
 * e.g. a row of dots meaning "a value is already saved". Never part
 * of the real value -- see the comment on the GTK implementation. */
void dlg_editbox_set_placeholder(dlgcontrol *ctrl, dlgparam *dp,
                                 char const *text);
/* Local patch: briefly flash a pushbutton's label green with a
 * checkmark ("<check> Saved!") instead of a separate status line. */
void dlg_button_flash_success(dlgcontrol *ctrl, dlgparam *dp,
                              char const *text);
/* Local patch: session export/import UI (unix/dialog.c), driving
 * export_sessions()/import_sessions() (putty.h). Neither refreshes
 * any listbox itself -- after a successful import, the caller must
 * re-scan (get_sesslist()) and refresh its own session listbox, since
 * only the caller (config.c) owns that cache and knows when it's
 * stale. */
...
void dlg_listbox_set_icon(dlgcontrol *ctrl, dlgparam *dp, int index,
                          const char *iconname);
/* Local patch: like dlg_listbox_set_icon, but also shows a small
 * green/red dot over the icon once it's known whether `host':`port'
 * is reachable -- and kicks off that check asynchronously (never
 * blocks) the first time a given `sessionname' is seen. sessionname
 * is the cache key that avoids re-checking on every listbox repaint
 * (e.g. every keystroke in the search box), so it must be stable and
 * unique per row -- the listbox's own untranslated display name is
 * the right thing to pass. host may be NULL/empty (e.g. a session
 * that's never had a host set), in which case no check is started and
 * no dot is ever shown, same as before this existed. */
void dlg_listbox_set_icon_and_check(dlgcontrol *ctrl, dlgparam *dp,
                                    int index, const char *iconname,
                                    const char *sessionname,
                                    const char *host, int port);
/* Local patch: "no internet on this machine" warning, shown next to
 * the Open/Cancel buttons instead of miscolouring every session dot
 * red when it's really just this machine that's offline. See the
 * ROWSTATUS_NETDOWN comment in unix/dialog.c for the detection logic;
 * these three just expose enough of it for config.c to wire up its
 * own ctrl_text() warning control. */
