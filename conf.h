...
CONF_OPTION(ui_language,
    VALUE_TYPE(INT),
    DEFAULT_INT(0),                    /* 0 = English, 1 = Russian */
    SAVE_KEYWORD("UILanguage"),
)
...
/* Local patch: per-session distro/OS icon shown in the saved-sessions list. */
CONF_OPTION(distro_icon,
    VALUE_TYPE(STR),
    DEFAULT_STR(""),
    SAVE_KEYWORD("DistroIcon"),
)

/* Local patch: auto-login password. Holds an AES-256-CBC-encrypted blob
 * (see unix/pwcrypt.c), never plaintext -- fed to the cmdline-password
 * mechanism (decrypted in memory only) at connect, ignored if a key is
 * set. CONF_password_new is a transient, never-saved holder for what the
 * user is currently typing in the box; see password_handler() in
 * config.c and commit_password_encryption(). */
CONF_OPTION(password,
    VALUE_TYPE(STR),
    DEFAULT_STR(""),
    SAVE_KEYWORD("Password"),
)
CONF_OPTION(password_new,
    VALUE_TYPE(STR),
    DEFAULT_STR(""),
    NOT_SAVED,
)

/* Local patch: global (profile-independent) appearance settings. These
 * live in ~/.putty/global-appearance, not in the session file -- the
 * Conf fields below only exist so the "Оформление" panel has something
 * to bind its controls to while editing. */
CONF_OPTION(bg_alpha,
    VALUE_TYPE(INT),
    DEFAULT_INT(82),
    SAVE_KEYWORD("BgAlpha"),
)
CONF_OPTION(win_accent,
    VALUE_TYPE(STR),
    DEFAULT_STR("3DAEE9"),
    SAVE_KEYWORD("WinAccent"),
)
CONF_OPTION(win_bg,
    VALUE_TYPE(STR),
    DEFAULT_STR("1E1E20"),
...
/* Local patch: opacity of the *settings dialog* window itself --
 * independent of bg_alpha above, which is the terminal's. */
CONF_OPTION(win_alpha,
    VALUE_TYPE(INT),
    DEFAULT_INT(92),
    SAVE_KEYWORD("WinAlpha"),
)
/* Local patch: font used for the settings dialog's own UI (labels,
 * buttons, tree, ...) -- independent of "font" above, which is the
 * terminal's. Same VALUE_TYPE(FONT) as "font", no DEFAULT_STR, for
 * the same reason: FontSpec fields don't take one. */
CONF_OPTION(win_font,
    VALUE_TYPE(FONT),
    SAVE_KEYWORD("WinFont"),
)
